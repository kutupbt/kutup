...
| 4 | Web contact privacy and trust | **In progress:** client-only durable message requests/blocking and Signal-style encrypted profiles are implemented and live verified. Key transparency transactionally logs every manifest, authenticates the current value with a sparse Merkle map, and supplies inclusion/consistency/map proofs across local and federated reads. Every exact log/map head now has a persistent dedicated operator signature; a separately deployed witness verifies consistency from its own durable state and attests that head, while client policies enforce pinned operator/witness keys and quorum before session mutation. Local HTTPS, an isolated quorum-1 witness topology, and the two-server offline/restart federation harness are live verified. Remaining trust work: scheduled/range monitoring, cross-witness gossip/auditing, and authenticated remote policy distribution/rotation; then sealed sender with its delivery-capability abuse gate. |
