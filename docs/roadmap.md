...
| 5 | Unified MLS conversations and web private groups | **In progress, unadvertised:** RFC 9420 suite `0x0002`, durable OpenMLS lifecycle, manifest-bound KeyPackages/Welcome/Commit/application processing, mandatory group-private role/control state, multi-authority BFT control log, restart-safe authority and new-participant bootstrap, creator-only group genesis, destination-private membership transitions, identified 30-day invitation decisions, and an incarnation-bound cursor/ack MLS mailbox are implemented. Browser flows now cover authenticated ordering-policy retrieval, atomic genesis, restart-safe add/remove and administrator finalization, complete quorum-history replay before Welcome installation, ordered inbound control Commits, anonymous application messages, and group management UI. Owner-approved authority replacement is implemented as one unchanged-roster MLS Commit with durable old/new quorum stages, automatic new-authority history bootstrap, exact participant delivery, restart reconciliation, and exact authority/quorum UI. Its isolated two-server remove/re-add, restart, and continued-delivery browser gate passes. Owner-set changes persist a partial initiating certificate, exchange exact owner-only approval requests/responses inside MLS, survive requester and approver restarts, block ordering below the current-owner quorum, and expose explicit web approve/reject controls. Their two-server browser gate proves the complete approval/finalization path and continued delivery. Owner-approved closure now uses the same private approval path and one unchanged-roster terminal Commit; its gate proves no ordering before quorum, closure on both domains, send blocking, and closed-history persistence after reload. Authenticated manifest proofs with gap recovery, epoch capabilities, RFC 9180 HPKE delivery, WASM bindings, privacy-bounded telemetry, and admin cryptographic inspection routes are also implemented. Remaining gates: recovery and suite/policy owner actions; linked-device MLS state synchronization; federated invitation-rejection feedback; remaining adversarial/scale E2E; and capability advertisement. V1 supports at least 256 and policy may allow 1000. See `docs/chat-mls.md`. |
