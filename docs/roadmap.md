...
| 6 | Web messaging and media | **Chat-media is advertised and clean two-server verified; encrypted replies are implemented and browser verified.** Direct, Note-to-Self and MLS attachments use one immutable Rust/WASM-secretstream object, resumable tus upload, sender-free durable federation copies, shared account quota, manual streaming download, clearable encrypted per-conversation accounting and restart-safe linked-device rebuild. Replies bind a canonical logical message UUID only inside the shared E2EE content and survive linked-device history recovery. The server limit defaults to the V1 2 GiB ceiling and may only be lowered. Reactions, edits/deletes, receipts, typing, disappearing messages, local search, capture UX and voice-note recording remain the next Phase 6 slices. See `docs/chat-media.md`. |
