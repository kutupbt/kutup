...
| 4 | Web contact privacy and trust | **In progress:** client-only durable message requests and blocking are implemented in the shared engine/WASM and web request inbox, including encrypted convergent linked-device controls and blocked-plaintext suppression after ratchet advancement. Next: profiles/profile-key rotation, key transparency, then sealed sender with its delivery-capability abuse gate. |
