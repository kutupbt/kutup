...
| 3 | Web federation foundation | **Implemented and two-server live verified:** canonical `username@server`, typed conversations, persistent server signing identity, `.well-known` delegation, destination/body-bound signed requests, replay-safe remote manifest/PQ-bundle discovery, durable per-destination in-order delivery, device-mismatch recovery, terminal rejection, and explicit sequence-gap replay. The isolated harness also proves retry survives an origin process restart while the destination is offline. Administrator-managed `disabled`/`allowlist`/`blocklist`/`open` admission, directional per-domain rules, audit logging, and responsive admin UI are implemented; disabled policy also suppresses discovery/capability advertisement. Fresh databases default to allowlist while existing databases with users migrate to open. Federation is advertised only when the signing identity is configured and policy permits it. No alias namespace. **Unified-stack progress:** the crypto-neutral federation v2 protocol crate, typed auth profile, signed identity rotation/discovery, strict RFC 9421/9530 request/response authentication, and golden vectors are implemented and documented in `docs/federation-protocol.md`; generic persistence and the atomic runtime cut-over remain. |
| 4 | Web contact privacy and trust | **In progress:** client-only durable message requests/blocking and Signal-style encrypted profiles are implemented and live verified. Key transparency transactionally logs every manifest, authenticates the current value with a sparse Merkle map, and supplies inclusion/consistency/map proofs across local and federated reads. Every exact log/map head has a persistent operator signature; a separately deployed witness verifies consistency and attests it, while client policies enforce pinned operator/witness keys and quorum. The web client now independently monitors its local head on open/online/foreground/reconnect and every 15 visible minutes, persists healthy/unavailable/verification-failed state, displays it, and blocks new sends after verification failure. Production WASM/web plus the full Chromium messaging E2E, local HTTPS contract, isolated quorum-1 witness topology, and two-server offline/restart federation harness are live verified. **Next:** authenticated remote identity/operator/witness pinning and rotation, then scheduled remote monitoring. Remaining later trust work: skipped-update/range proofs and cross-witness gossip/auditing; then sealed sender with its delivery-capability abuse gate. |
