...
| 2b | Client slice: `kutup-chat-core` engine (wraps libsignal, native + wasm) + minimal 1:1 web UI | **In progress** — the native proof engine, durable outbox, 409 recovery, and drain/ack loop are complete on `claude/chat-phase1`; architecture hardening, async IndexedDB boundary, signed manifests, bindings, and UI remain. No UI ships before the hardened path works end-to-end. |
