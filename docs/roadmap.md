...
| 3 | Web federation foundation | **Implemented and two-server live verified:** canonical `username@server`, typed conversations, persistent server signing identity, `.well-known` delegation, destination/body-bound signed requests, replay-safe remote manifest/PQ-bundle discovery, durable per-destination in-order delivery, device-mismatch recovery, terminal rejection, and explicit sequence-gap replay. The isolated harness also proves retry survives an origin process restart while the destination is offline. Federation is advertised only when the signing identity is configured. No alias namespace. |
| 4 | Web contact privacy and trust | **In progress:** client-only durable message requests/blocking and Signal-style encrypted profiles are implemented and live verified. Key transparency now transactionally logs every manifest, authenticates the current account value with a sparse Merkle map whose root is the log's final leaf, supplies inclusion/consistency/map proofs across local and federated reads, and durably pins global homeserver checkpoints plus per-account event positions before session mutation. This detects rollback/history rewriting and stale-value selection inside a served checkpoint, but it does not resist operator-created split views. Next: periodic/range monitoring and independent auditing/witnessing or checkpoint gossip, then sealed sender with its delivery-capability abuse gate. |
