...
| 2b | Client slice: `kutup-chat-core` engine (wraps libsignal, native + wasm) + minimal 1:1 web UI | **In progress — web slice implemented and live-stack verified on `codex/chat-architecture-hardening`.** Complete: durable typed inbound journal/quarantine, async DB boundary, SQLCipher fail-closed native store, crash-safe registration and prekey refill, signed manifests, atomic IndexedDB, WASM transport facade, capability-gated bilingual 1:1 UI, Signal-style Note to Self with encrypted linked-device sent transcripts, Web Locks cross-tab serialization, WS-hint/REST reconciliation, outbound history, a passing Playwright two-account encrypted exchange + IndexedDB reload test, and a generated/host-smoke-tested UniFFI 0.32 Swift/Kotlin boundary in `kutup-client-ffi`. Ordinary outgoing-message transcript sync, XCFramework/AAR cross-compilation, Keychain/Keystore, and app integration in `kutup-ios`/`kutup-android` remain phase-2b gates. |
