...
| 4 | Web contact privacy and trust | **Implemented and two-server live verified:** durable message requests/blocking and Signal-style encrypted profiles; authenticated local/remote transparency policy histories and 15-minute restart-safe monitoring; checkpoint-bound multi-page skipped-manifest recovery; bounded witness views, shared server/standalone cross-view auditing, immutable fork evidence, and explicit audited recovery; contacts-only libsignal sealed sender with offline-root/online-certificate policy, transparency-bound identity validation, database-backed capability/origin rate limits, anonymous local/federated delivery, capability rotation on block, and no identified fallback. Rust/native/WASM and production web build gates pass; the expanded two-server browser/Compose gate proves remote policy pinning, established sealed delivery, capability invalidation, and no identified fallback. |
| 5 | Unified MLS conversations and web private groups | **In progress, unadvertised:** RFC 9420 suite `0x0002`, durable OpenMLS lifecycle, manifest-bound KeyPackages/Welcome/Commit/application processing, mandatory group-private role/control state, multi-authority BFT control log, restart-safe authority and new-participant bootstrap, creator-only group genesis, destination-private membership transitions, identified 30-day invitation decisions, and an incarnation-bound cursor/ack MLS mailbox are implemented. Browser flows now cover authenticated ordering-policy retrieval, atomic genesis, restart-safe add/remove and administrator finalization, complete quorum-history replay before Welcome installation, ordered inbound control Commits, anonymous application messages, and group management UI. Owner-approved authority replacement is implemented as one unchanged-roster MLS Commit with durable old/new quorum stages, automatic new-authority history bootstrap, exact participant delivery, restart reconciliation, and exact authority/quorum UI. Its isolated two-server remove/re-add, restart, and continued-delivery browser gate passes. Owner-set changes persist a partial initiating certificate, exchange exact owner-only approval requests/responses inside MLS, survive requester and approver restarts, block ordering below the current-owner quorum, and expose explicit web approve/reject controls. Their two-server browser gate proves the complete approval/finalization path and continued delivery. Owner-approved closure now uses the same private approval path and one unchanged-roster terminal Commit; its gate proves no ordering before quorum, closure on both domains, send blocking, and closed-history persistence after reload. Owner-approved incarnation recovery now preserves the exact roster/owners in a fresh GroupId and epoch-one genesis, stores immutable signed evidence, retries destination-private replicas durably, archives the old state atomically, and has WASM/browser initiation, manual approval, recipient verification, and admin inspection. Its isolated two-server gate proves requester/approver restart recovery, authenticated evidence retrieval, no finalization before quorum, post-recovery delivery, and reload persistence. Owner-approved private authorization and cryptographic-policy changes now use contiguous GroupContext state, unchanged-roster delivery, restart-safe manual approval, browser controls, administrator-only sender enforcement, and a non-downgradable user-message plaintext ceiling. Their two-server gate proves exact remote policy pinning, manual quorum, restricted sending, oversized-message rejection before transport, control availability under the tightened policy, member removal, quorum recovery, restart persistence, and continued delivery. Invitation rejection/expiry now creates canonical append-only feedback, retries it through authenticated federation, verifies the exact finalized Welcome at the invitation origin, exposes it only to active group administrators, and never permits automatic server-side roster mutation. The two-server browser gate proves cross-server rejection, authenticated warning visibility, and explicit administrator removal through MLS. Linked-device MLS synchronization now gives each installation an independent manifest-bound leaf, orders exact add/remove device sets through `DeviceSync`, auto-installs only history-verified Welcomes for already-active accounts, and retains immutable removal history. The full two-server browser gate proves second-installation join, anonymous messages in both directions, and reload persistence. Authenticated manifest proofs with gap recovery, epoch capabilities, RFC 9180 HPKE delivery, WASM bindings, privacy-bounded telemetry, and admin cryptographic inspection routes are also implemented. The adversarial/scale gate now passes with a real 256-member OpenMLS tree, signed fork and malformed-page cases, raw two-server replay/enumeration/rotated-capability probes, and destination-log privacy verification. Remaining gates: exact authority-policy/fingerprint browser UI and explicit capability advertisement. V1 supports at least 256 and policy may allow 1000. See `docs/chat-mls.md`. |
