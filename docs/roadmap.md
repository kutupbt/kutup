...
| 3 | Web federation foundation | **Implemented locally; two-server live verification pending:** canonical `username@server`, typed conversations, persistent server signing identity, `.well-known` delegation, destination/body-bound signed requests, replay-safe remote manifest/PQ-bundle discovery, durable per-destination in-order delivery, device-mismatch recovery, and explicit sequence-gap replay. Federation is advertised only when the signing identity is configured. No alias namespace. |
