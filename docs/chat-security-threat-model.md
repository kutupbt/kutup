...
| Unwanted, unready, expired, or replayed group invitation | Creator-only group genesis, identified pending membership, 30-day expiry, explicit accept/reject, no pending-member authorization or capability publication, an origin-scoped server receipt, and an MLS-encrypted acceptance bound to the verified sender and exact client-private join epoch | The administrator composer remains blocked until later invitees are ready. Plaintext acceptance is not broadcast to unrelated participant servers, and removal/re-addition cannot reuse an old receipt. Reject/expiry deletes membership-control mailbox material and is audited; an administrator must commit removal before a rejected account leaves the MLS roster. |
