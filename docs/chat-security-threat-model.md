...
| Browser storage is cleared or a disposable profile is abandoned | A replacement installation gets independent device keys; active-device review/revocation and an existing-device-authenticated E2EE history transfer are mandatory | The server cannot copy old per-device ciphertext. With no surviving authorized device or E2EE backup, the UI reports history as unrecoverable instead of silently showing an empty inbox. |
