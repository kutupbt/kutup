...
- **License path: opt-in install script.** kutup is AGPL-3.0-only. Operators run `./install-onlyoffice.sh` to fetch AGPL OnlyOffice client JS into `frontend/public/onlyoffice/dist/` (gitignored). Same model CryptPad uses; bridge files carry the upstream `AGPL-3.0-or-later` SPDX header.
