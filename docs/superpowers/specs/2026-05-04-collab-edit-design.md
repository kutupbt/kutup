...
- License path for bundling AGPL OnlyOffice client — kutup is AGPL-3.0-only, so the integration is license-compatible; the office subtree carries `AGPL-3.0-or-later` SPDX headers per the OnlyOffice client's terms.
...
6. **License compatibility with OnlyOffice** — resolved: kutup is AGPL-3.0-only and the office subtree (`frontend/public/onlyoffice/`, `frontend/src/components/editors/office/`) carries the `AGPL-3.0-or-later` SPDX header per the OnlyOffice client's upstream terms. OnlyOffice's actual JS is fetched at install time and lives gitignored under `frontend/public/onlyoffice/dist/`, never vendored.
