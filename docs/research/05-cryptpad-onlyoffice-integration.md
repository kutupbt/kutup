...
**Implication for kutup:** kutup is currently MIT-licensed. Bundling AGPL OnlyOffice JS would require either (a) changing kutup's license (or relicensing the office-edit subdirectory) to AGPL, or (b) loading OnlyOffice JS at runtime from a separate AGPL package without static-linking it — needs lawyer-level scrutiny. **This is a real licensing decision the user must make.**
