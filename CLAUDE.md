...
- **The crypto must stay mirrored**: `frontend/src/crypto/` (canonical, libsodium-wrappers), `cmd/kutup/internal/crypto/` (Go), and `crates/kutup-crypto/` (Rust — the Go→Rust rewrite, see below) implement the same primitives (KDF, secretstream framing, mnemonic, asymmetric) — keep them in sync when you change one. The Rust crate is verified byte-for-byte against the Go reference via vectors emitted by `cmd/kutup/tools/genvectors` + `backend/tools/genvectors` and checked in `crates/kutup-crypto/tests/vectors.rs`; regenerate the vectors and re-run `cargo test -p kutup-crypto` after any crypto change.
- **Go→Rust rewrite in progress** (branch `claude/go-rust-rewrite-G16zO`): the Go backend (`backend/`) and CLI (`cmd/kutup/`) are being rewritten in Rust under a Cargo workspace at the repo root (`crates/`), in the order crypto → CLI → backend. `src-tauri/` is intentionally excluded from the workspace. Shared decisions: Axum + utoipa (server), sqlx (Postgres, existing migrations unchanged), `dryoc` + RustCrypto for crypto. The Go code remains the source of truth until each Rust replacement passes its parity/test gate.
