...
fn one_to_one_text_round_trip_through_wire_types() {
    ...
    alice.establish(&bob_addr, &bob_bundle, &mut rng).unwrap();
    let msg1 = ChatContent::text("2026-07-13T10:00:00Z", 1, "hello bob 👋");
    let env1 = alice.encrypt(&bob_addr, bob_reg, &msg1, &mut rng).unwrap();
    ...
    let got1 = bob.decrypt(&alice_addr, &delivered1, &mut rng).unwrap();
    ...
    let got2 = alice.decrypt(&bob_addr, &delivered2, &mut rng).unwrap();
    ...
    for round in 2..8u64 {
        ...
        let e = alice.encrypt(&bob_addr, bob_reg, &m, &mut rng).unwrap();
        let d = bob
            .decrypt(&alice_addr, &wrap(e, "alice", round + 10), &mut rng)
            .unwrap();
        assert_eq!(d.as_text().unwrap().text, format!("a->b {round}"));

        let m = ChatContent::text("t", round, format!("b->a {round}"));
        let e = bob.encrypt(&alice_addr, alice_reg, &m, &mut rng).unwrap();
        let d = alice
            .decrypt(&bob_addr, &wrap(e, "bob", round + 20), &mut rng)
            .unwrap();
...
fn tampered_ciphertext_is_rejected() {
    ...
    assert!(
        bob.decrypt(&alice_addr, &delivered, &mut rng).is_err(),
