//! Owner-quorum-authorized MLS owner-set transitions.
//!
//! Owner identities are group-scoped Ed25519 pseudonyms. Account bindings stay
//! exclusively in the MLS-encrypted private roster while ordering authorities
//! verify the pseudonymous owner certificate and committed delivery digests.

use super::*;
use ed25519_dalek::{Signer as _, SigningKey as Ed25519SigningKey};
use kutup_chat_proto::{MlsOwnerApprovalCertificateV1, MlsOwnerApprovalV1, MlsOwnerChangeV1};

/// Exact durable retry material for one owner-set transition.
#[derive(Clone, Debug, PartialEq, Eq, Serialize, Deserialize)]
#[serde(rename_all = "camelCase", deny_unknown_fields)]
pub struct PendingMlsOwnerChange {
    pub mls_group_id: Vec<u8>,
    pub next_roster: Vec<MlsConversationMemberV1>,
    pub deliveries: Vec<MlsMembershipDeliveryV1>,
    pub owner_change: MlsOwnerChangeV1,
    pub vote_request: FederatedMlsOrderingVoteRequestV1,
    pub commit_hash: String,
    #[serde(skip_serializing_if = "Option::is_none")]
    pub final_request: Option<CommitMlsControlBlockV1>,
}

/// Atomic result of staging the MLS Commit and owner-governance data.
#[derive(Clone, Debug, PartialEq, Eq, Serialize, Deserialize)]
#[serde(rename_all = "camelCase", deny_unknown_fields)]
pub struct PreparedMlsOwnerChange {
    pub pending: PendingMlsCommit,
    pub control: PendingMlsOwnerChange,
}

/// State returned after the authority quorum acknowledges the exact block.
#[derive(Clone, Debug, PartialEq, Eq, Serialize, Deserialize)]
#[serde(rename_all = "camelCase", deny_unknown_fields)]
pub struct FinalizedMlsOwnerChange {
    pub group: LocalMlsGroupState,
    pub conversation: LocalMlsConversationRecord,
}

impl PendingMlsOwnerChange {
    pub(super) fn validate_durable(&self) -> Result<()> {
        validate_group_id(&self.mls_group_id)?;
        validate_sha256_hex("MLS owner commit hash", &self.commit_hash)?;
        self.owner_change
            .validate()
            .map_err(|error| ChatError::Db(format!("invalid durable owner change: {error}")))?;
        self.vote_request
            .validate()
            .map_err(|error| ChatError::Db(format!("invalid durable owner vote: {error}")))?;
        let block = &self.vote_request.block;
        let transition = &self.owner_change.delivery_transition;
        if block.proposal.action_type != MlsControlActionTypeV1::OwnerSetChange
            || self.vote_request.previous_set_certificate.is_some()
            || self.vote_request.authority_change.is_some()
            || block.proposal.payload_digest != self.commit_hash
            || block.transition_digest.as_deref()
                != Some(
                    self.owner_change
                        .transition_digest()
                        .map_err(ChatError::Db)?
                        .as_str(),
                )
            || self.deliveries.len() != transition.deliveries.len()
            || roster_commitment(&self.next_roster).map_err(ChatError::Db)?
                != transition.next_roster_commitment
        {
            return Err(ChatError::Db(
                "durable MLS owner control fields are inconsistent".into(),
            ));
        }
        validate_owner_set_matches_roster(&self.owner_change.next_owner_set, &self.next_roster)
            .map_err(ChatError::Db)?;
        let certificate = block
            .owner_approval
            .as_ref()
            .ok_or_else(|| ChatError::Db("durable owner change has no owner quorum".into()))?;
        if certificate.owner_set_sequence == 0
            || certificate.approvals.is_empty()
            || certificate.proposal_hash != block.proposal.proposal_hash().map_err(ChatError::Db)?
            || certificate.transition_digest.as_deref() != block.transition_digest.as_deref()
        {
            return Err(ChatError::Db(
                "durable owner approval certificate is structurally inconsistent".into(),
            ));
        }
        for (delivery, commitment) in self.deliveries.iter().zip(&transition.deliveries) {
            delivery
                .verify_transition(transition)
                .map_err(ChatError::Db)?;
            if delivery.destination != commitment.destination
                || delivery.epoch_after != block.epoch_after
            {
                return Err(ChatError::Db(
                    "durable MLS owner delivery differs from its block".into(),
                ));
            }
        }
        if let Some(request) = &self.final_request {
            request.validate_shape().map_err(ChatError::Db)?;
            if request.finalized.block != *block
                || request.owner_change.as_ref() != Some(&self.owner_change)
                || request.membership_transition.is_some()
                || request.authority_change.is_some()
                || request.authority_transition.is_some()
            {
                return Err(ChatError::Db(
                    "durable finalized owner request differs from its retry record".into(),
                ));
            }
            request
                .finalized
                .verify(&self.vote_request.authority_set)
                .map_err(ChatError::Db)?;
        }
        Ok(())
    }
}

impl MlsClient {
    /// Create or return this account's stable, group-scoped prospective owner
    /// credential. Generating a candidate does not grant any role.
    pub async fn ensure_owner_candidate(
        &self,
        mls_group_id: &[u8],
        created_at_seconds: i64,
    ) -> Result<MlsOwnerCandidateV1> {
        validate_group_id(mls_group_id)?;
        if created_at_seconds < 0 {
            return Err(ChatError::Invalid("MLS candidate clock is invalid".into()));
        }
        let group_key = BASE64.encode(mls_group_id);
        let (provider, mut metadata) = self.load_provider().await?;
        let conversation = active_conversation_for_group(&metadata, mls_group_id)?.clone();
        let (local_address, _) = parse_device_credential_identity(&metadata.credential_identity)?;
        let account: AccountAddress =
            local_address
                .parse()
                .map_err(|error: kutup_chat_proto::AddressError| {
                    ChatError::Invalid(error.to_string())
                })?;
        if let Some(existing) = metadata
            .owner_candidates
            .get(&group_key)
            .and_then(|candidates| candidates.get(&local_address))
        {
            existing.verify().map_err(ChatError::Db)?;
            return Ok(existing.clone());
        }
        if !conversation
            .current_roster
            .iter()
            .any(|member| member.address.canonical() == local_address)
        {
            return Err(ChatError::Trust(
                "only a current group member can create an owner candidate".into(),
            ));
        }
        let seed = if let Some(seed) = metadata.group_owner_private_keys.get(&group_key) {
            seed.clone()
        } else if let Some(seed) = metadata.group_owner_candidate_private_keys.get(&group_key) {
            seed.clone()
        } else {
            let signer = Ed25519SigningKey::generate(&mut OsRng);
            let seed = signer.to_bytes().to_vec();
            metadata
                .group_owner_candidate_private_keys
                .insert(group_key.clone(), seed.clone());
            seed
        };
        let seed: [u8; 32] = seed
            .try_into()
            .map_err(|_| ChatError::Db("invalid owner candidate seed".into()))?;
        let signer = Ed25519SigningKey::from_bytes(&seed);
        let public_key = signer.verifying_key().to_bytes();
        let mut candidate = MlsOwnerCandidateV1 {
            protocol_version: MLS_PROTOCOL_VERSION,
            conversation_id: conversation.request.genesis.conversation_id,
            incarnation: conversation.request.genesis.incarnation,
            account,
            owner_id: hex::encode(Sha256::digest(public_key)),
            public_key: BASE64.encode(public_key),
            created_at: created_at_seconds,
            signature: String::new(),
        };
        candidate.signature = BASE64.encode(
            signer
                .sign(&candidate.signing_bytes().map_err(ChatError::Protocol)?)
                .to_bytes(),
        );
        candidate.verify().map_err(ChatError::Protocol)?;
        metadata
            .owner_candidates
            .entry(group_key)
            .or_default()
            .insert(local_address, candidate.clone());
        let state = snapshot_provider(&provider, &metadata)?;
        self.db
            .apply(&Pending {
                mls_state: Some(state),
                ..Pending::default()
            })
            .await?;
        Ok(candidate)
    }

    pub async fn owner_candidates(&self, mls_group_id: &[u8]) -> Result<Vec<MlsOwnerCandidateV1>> {
        validate_group_id(mls_group_id)?;
        let (_, metadata) = self.load_provider().await?;
        Ok(metadata
            .owner_candidates
            .get(&BASE64.encode(mls_group_id))
            .map(|candidates| candidates.values().cloned().collect())
            .unwrap_or_default())
    }

    /// Encrypt the stable local candidate as an invisible MLS group-control
    /// application message and persist its normal anonymous-delivery outbox.
    pub async fn create_owner_candidate_message(
        &self,
        mls_group_id: &[u8],
        created_at_seconds: i64,
    ) -> Result<Option<MlsOutboxEntry>> {
        let candidate = self
            .ensure_owner_candidate(mls_group_id, created_at_seconds)
            .await?;
        let (_, metadata) = self.load_provider().await?;
        let conversation = active_conversation_for_group(&metadata, mls_group_id)?;
        let mut id_material = Vec::with_capacity(128);
        id_material.extend_from_slice(b"kutup/mls/owner-candidate-message/v1\0");
        id_material.extend_from_slice(candidate.conversation_id.as_bytes());
        id_material.extend_from_slice(&candidate.incarnation.to_be_bytes());
        id_material.extend_from_slice(&conversation.last_finalized_epoch.to_be_bytes());
        id_material.extend_from_slice(candidate.owner_id.as_bytes());
        let digest = Sha256::digest(id_material);
        let mut uuid_bytes = [0u8; 16];
        uuid_bytes.copy_from_slice(&digest[..16]);
        uuid_bytes[6] = (uuid_bytes[6] & 0x0f) | 0x50;
        uuid_bytes[8] = (uuid_bytes[8] & 0x3f) | 0x80;
        let send_id = Uuid::from_bytes(uuid_bytes).to_string();
        if let Some(existing) = self.db.load_mls_outbox(&send_id).await? {
            return Ok(Some(existing));
        }
        if let Some(history) = self.db.load_mls_message(&format!("out:{send_id}")).await? {
            let content: ChatContent = serde_json::from_slice(&history.content)
                .map_err(|error| ChatError::Db(error.to_string()))?;
            let body: MlsGroupControlBodyV1 = serde_json::from_value(content.body)
                .map_err(|error| ChatError::Db(error.to_string()))?;
            if !history.outgoing
                || !history.delivered
                || history.message_id != send_id
                || history.conversation_id != *candidate.conversation_id.as_bytes()
                || history.incarnation != candidate.incarnation
                || history.mls_group_id != mls_group_id
                || content.kind != kutup_chat_proto::content::kind::GROUP_CONTROL
                || body
                    != (MlsGroupControlBodyV1::OwnerCandidate {
                        candidate: candidate.clone(),
                    })
            {
                return Err(ChatError::Db(
                    "durable MLS owner-candidate receipt differs from its deterministic id".into(),
                ));
            }
            return Ok(None);
        }
        let (local_address, _) = parse_device_credential_identity(&metadata.credential_identity)?;
        let expected_recipients = conversation
            .current_roster
            .iter()
            .map(|member| member.address.canonical())
            .filter(|address| address != &local_address)
            .collect::<Vec<_>>();
        if expected_recipients.is_empty() {
            return Err(ChatError::Invalid(
                "MLS candidate has no remote group recipient".into(),
            ));
        }
        let seq = self
            .db
            .load_last_sent_seq()
            .await?
            .unwrap_or(0)
            .checked_add(1)
            .ok_or_else(|| ChatError::Invalid("MLS sender sequence overflow".into()))?;
        let content = ChatContent {
            v: ChatContent::VERSION,
            kind: kutup_chat_proto::content::kind::GROUP_CONTROL.into(),
            sent_at: candidate.created_at.to_string(),
            seq,
            message_id: Some(send_id.clone()),
            profile_key: None,
            body: serde_json::to_value(MlsGroupControlBodyV1::OwnerCandidate { candidate })
                .map_err(|error| ChatError::Content(error.to_string()))?,
            extra: serde_json::Map::new(),
        };
        let content_bytes =
            serde_json::to_vec(&content).map_err(|error| ChatError::Content(error.to_string()))?;
        let created_at_ms = created_at_seconds
            .checked_mul(1000)
            .ok_or_else(|| ChatError::Invalid("MLS candidate clock overflow".into()))?;
        self.create_application_message_inner(
            &send_id,
            *candidate_conversation_id(&content)?.as_bytes(),
            conversation.request.genesis.incarnation,
            mls_group_id,
            &content_bytes,
            &content_bytes,
            expected_recipients,
            created_at_ms,
            Some(seq),
        )
        .await
        .map(Some)
    }

    /// Stage an owner-role-only private roster transition and authorize it with
    /// this device's current group-scoped owner key.
    pub async fn prepare_owner_change(
        &self,
        mls_group_id: &[u8],
        proposal_id: Uuid,
        next_roster: &[MlsConversationMemberV1],
        next_owner_set: MlsOwnerSetV1,
        created_at_seconds: i64,
    ) -> Result<PreparedMlsOwnerChange> {
        validate_group_id(mls_group_id)?;
        if proposal_id.is_nil() || created_at_seconds < 0 {
            return Err(ChatError::Invalid(
                "MLS owner change requires a proposal id and valid clock".into(),
            ));
        }
        next_owner_set.validate().map_err(ChatError::Invalid)?;
        let group_key = BASE64.encode(mls_group_id);
        let (provider, mut metadata) = self.load_provider().await?;
        if metadata.pending_owner_changes.contains_key(&group_key) {
            return Err(ChatError::Trust(
                "an MLS owner change is already pending".into(),
            ));
        }
        if metadata.pending_commits.contains_key(&group_key)
            || metadata.pending_membership_changes.contains_key(&group_key)
            || metadata.pending_authority_changes.contains_key(&group_key)
            || metadata.pending_closes.contains_key(&group_key)
            || metadata.pending_policy_changes.contains_key(&group_key)
            || metadata.pending_recoveries.contains_key(&group_key)
        {
            return Err(ChatError::Trust(
                "another MLS control operation is already pending".into(),
            ));
        }
        let conversation = active_conversation_for_group(&metadata, mls_group_id)?.clone();
        if conversation.current_owner_set.sequence.checked_add(1) != Some(next_owner_set.sequence) {
            return Err(ChatError::Invalid(
                "MLS owner-set sequence must advance by exactly one".into(),
            ));
        }
        validate_owner_role_transition(
            &conversation.current_roster,
            next_roster,
            &conversation.current_owner_set,
            &next_owner_set,
        )?;
        validate_new_owner_candidates(
            &metadata,
            &conversation,
            next_roster,
            &next_owner_set,
            &group_key,
        )?;
        let (local_address, _) = parse_device_credential_identity(&metadata.credential_identity)?;
        let local_member = conversation
            .current_roster
            .iter()
            .find(|member| member.address.canonical() == local_address)
            .ok_or_else(|| {
                ChatError::Trust("local account is absent from the MLS roster".into())
            })?;
        let owner = group_owner_credential(&metadata, mls_group_id)?;
        if local_member.owner_id.as_deref() != Some(owner.owner_id.as_str())
            || conversation
                .current_owner_set
                .owner(&owner.owner_id)
                .is_none()
        {
            return Err(ChatError::Trust(
                "this device has no current owner credential for the owner change".into(),
            ));
        }

        let group_id = GroupId::from_slice(mls_group_id);
        let group = MlsGroup::load(provider.storage(), &group_id)
            .map_err(|error| mls_error("load MLS group", error))?
            .ok_or_else(|| {
                ChatError::MissingKeyMaterial("MLS group state is unavailable".into())
            })?;
        ensure_v1_group(&group)?;
        if group.epoch().as_u64() != conversation.last_finalized_epoch {
            return Err(ChatError::Trust(
                "OpenMLS epoch differs from the pinned control-log epoch".into(),
            ));
        }
        ensure_private_control_matches_record(group.extensions(), &conversation)?;
        let current_devices = group
            .members()
            .map(|member| {
                let identity = std::str::from_utf8(member.credential.serialized_content())
                    .map_err(|_| ChatError::Trust("MLS credential identity is not UTF-8".into()))?;
                parse_device_credential_identity(identity)
            })
            .collect::<Result<Vec<_>>>()?;
        let next_height = conversation
            .last_finalized_height
            .checked_add(1)
            .ok_or_else(|| ChatError::Invalid("MLS control height is exhausted".into()))?;
        let next_epoch = conversation
            .last_finalized_epoch
            .checked_add(1)
            .ok_or_else(|| ChatError::Invalid("MLS epoch is exhausted".into()))?;
        let next_private_control = MlsPrivateControlStateV1 {
            protocol_version: MLS_PROTOCOL_VERSION,
            conversation_id: conversation.request.genesis.conversation_id,
            incarnation: conversation.request.genesis.incarnation,
            proposal_id: Some(proposal_id),
            height: next_height,
            initial_epoch: conversation.request.genesis.initial_epoch,
            epoch: next_epoch,
            previous_block_hash: conversation.last_block_hash.clone(),
            genesis_roster: conversation.request.members.clone(),
            genesis_authority_set: conversation.request.genesis.authority_set.clone(),
            genesis_owner_set: conversation
                .request
                .genesis
                .owner_set
                .clone()
                .ok_or_else(|| ChatError::Db("group genesis has no owner set".into()))?,
            genesis_authorization_policy: conversation
                .genesis_authorization_policy
                .clone(),
            genesis_cryptographic_policy: conversation
                .genesis_cryptographic_policy
                .clone(),
            roster: next_roster.to_vec(),
            authority_set: conversation.current_authority_set.clone(),
            owner_set: next_owner_set.clone(),
            authorization_policy: conversation.current_authorization_policy.clone(),
            cryptographic_policy: conversation.current_cryptographic_policy.clone(),
        };
        next_private_control
            .validate()
            .map_err(ChatError::Invalid)?;
        let pending = stage_private_control_update(
            &provider,
            &mut metadata,
            mls_group_id,
            &next_private_control,
        )?;
        let (deliveries, delivery_transition) = super::governance::build_governance_deliveries(
            &metadata,
            &conversation,
            proposal_id,
            next_roster,
            &current_devices,
            &pending,
        )?;
        let owner_change = MlsOwnerChangeV1 {
            next_owner_set,
            delivery_transition,
        };
        let transition_digest = owner_change
            .transition_digest()
            .map_err(ChatError::Protocol)?;
        let proposal = sign_control_proposal_with_metadata(
            &metadata,
            mls_group_id,
            conversation.request.genesis.conversation_id,
            conversation.request.genesis.incarnation,
            proposal_id,
            pending.epoch_before,
            MlsControlActionTypeV1::OwnerSetChange,
            &pending.commit,
            created_at_seconds,
        )?;
        let proposal_hash = proposal.proposal_hash().map_err(ChatError::Protocol)?;
        let owner_seed: [u8; 32] = ensure_group_owner_key(&metadata, mls_group_id)?
            .try_into()
            .map_err(|_| ChatError::Db("invalid durable MLS owner seed".into()))?;
        let owner_signer = Ed25519SigningKey::from_bytes(&owner_seed);
        let mut approval = MlsOwnerApprovalV1 {
            conversation_id: proposal.conversation_id,
            incarnation: proposal.incarnation,
            owner_set_sequence: conversation.current_owner_set.sequence,
            proposal_hash: proposal_hash.clone(),
            transition_digest: Some(transition_digest.clone()),
            owner_id: owner.owner_id,
            approved_at: created_at_seconds,
            signature: String::new(),
        };
        approval.signature = BASE64.encode(
            owner_signer
                .sign(&approval.signing_bytes().map_err(ChatError::Protocol)?)
                .to_bytes(),
        );
        let owner_approval = MlsOwnerApprovalCertificateV1 {
            owner_set_sequence: conversation.current_owner_set.sequence,
            proposal_hash,
            transition_digest: Some(transition_digest.clone()),
            approvals: vec![approval],
        };
        owner_approval
            .verify_partial(
                &proposal,
                Some(transition_digest.as_str()),
                &conversation.current_owner_set,
            )
            .map_err(ChatError::Trust)?;
        let block = MlsControlBlockV1 {
            conversation_id: proposal.conversation_id,
            incarnation: proposal.incarnation,
            height: next_height,
            previous_block_hash: conversation.last_block_hash.clone(),
            epoch_before: pending.epoch_before,
            epoch_after: pending.epoch_after,
            proposal,
            transition_digest: Some(transition_digest),
            owner_approval: Some(owner_approval),
            finalized_at: created_at_seconds,
        };
        block.validate().map_err(ChatError::Protocol)?;
        let vote_request = FederatedMlsOrderingVoteRequestV1 {
            protocol_version: MLS_PROTOCOL_VERSION,
            block,
            authority_change: None,
            authority_set: conversation.current_authority_set.clone(),
            previous_set_certificate: None,
        };
        vote_request.validate().map_err(ChatError::Protocol)?;
        let control = PendingMlsOwnerChange {
            mls_group_id: mls_group_id.to_vec(),
            next_roster: next_roster.to_vec(),
            deliveries,
            owner_change,
            vote_request,
            commit_hash: pending.commit_hash.clone(),
            final_request: None,
        };
        validate_pending_owner_change(&control, &conversation.current_owner_set)?;
        metadata
            .pending_owner_changes
            .insert(group_key, control.clone());
        let state = snapshot_provider(&provider, &metadata)?;
        self.db
            .apply(&Pending {
                mls_state: Some(state),
                ..Pending::default()
            })
            .await?;
        Ok(PreparedMlsOwnerChange { pending, control })
    }

    pub async fn pending_owner_changes(&self) -> Result<Vec<PendingMlsOwnerChange>> {
        let (_, metadata) = self.load_provider().await?;
        Ok(metadata.pending_owner_changes.values().cloned().collect())
    }

    pub async fn build_owner_commit_request(
        &self,
        mls_group_id: &[u8],
        quorum_certificate: MlsOrderingQuorumCertificateV1,
    ) -> Result<CommitMlsControlBlockV1> {
        validate_group_id(mls_group_id)?;
        let (provider, mut metadata) = self.load_provider().await?;
        let conversation = active_conversation_for_group(&metadata, mls_group_id)?.clone();
        let control = metadata
            .pending_owner_changes
            .get_mut(&BASE64.encode(mls_group_id))
            .ok_or_else(|| ChatError::Trust("pending MLS owner control is unavailable".into()))?;
        control
            .vote_request
            .block
            .owner_approval
            .as_ref()
            .ok_or_else(|| ChatError::Trust("pending MLS owner control has no approvals".into()))?
            .verify(
                &control.vote_request.block.proposal,
                control.vote_request.block.transition_digest.as_deref(),
                &conversation.current_owner_set,
            )
            .map_err(|_| {
                ChatError::Trust(
                    "MLS owner quorum requires additional explicit owner approvals".into(),
                )
            })?;
        if let Some(request) = &control.final_request {
            if request.finalized.quorum_certificate != quorum_certificate {
                return Err(ChatError::Trust(
                    "different authority quorum already pinned for owner change".into(),
                ));
            }
            return Ok(request.clone());
        }
        quorum_certificate
            .verify(&control.vote_request.authority_set)
            .map_err(ChatError::Trust)?;
        let block = &control.vote_request.block;
        if quorum_certificate.block_hash != block.block_hash().map_err(ChatError::Protocol)?
            || quorum_certificate.height != block.height
        {
            return Err(ChatError::Trust(
                "authority quorum finalized a different owner block".into(),
            ));
        }
        let request = CommitMlsControlBlockV1 {
            finalized: MlsFinalizedControlBlockV1 {
                block: block.clone(),
                quorum_certificate,
            },
            membership_transition: None,
            authority_change: None,
            authority_transition: None,
            owner_change: Some(control.owner_change.clone()),
        };
        request.validate_shape().map_err(ChatError::Protocol)?;
        control.final_request = Some(request.clone());
        let state = snapshot_provider(&provider, &metadata)?;
        self.db
            .apply(&Pending {
                mls_state: Some(state),
                ..Pending::default()
            })
            .await?;
        Ok(request)
    }

    pub async fn finalize_owner_change(
        &self,
        mls_group_id: &[u8],
        acknowledgement: &CommitMlsControlBlockResponseV1,
    ) -> Result<FinalizedMlsOwnerChange> {
        validate_group_id(mls_group_id)?;
        let group_key = BASE64.encode(mls_group_id);
        let (provider, mut metadata) = self.load_provider().await?;
        let control = metadata
            .pending_owner_changes
            .get(&group_key)
            .cloned()
            .ok_or_else(|| ChatError::Trust("pending MLS owner control is unavailable".into()))?;
        let block = &control.vote_request.block;
        let block_hash = block.block_hash().map_err(ChatError::Protocol)?;
        if acknowledgement.conversation_id != block.conversation_id
            || acknowledgement.incarnation != block.incarnation
            || acknowledgement.height != block.height
            || acknowledgement.epoch != block.epoch_after
            || acknowledgement.block_hash != block_hash
            || control.final_request.is_none()
        {
            return Err(ChatError::Trust(
                "server acknowledged a different or incomplete MLS owner block".into(),
            ));
        }
        let pending = metadata
            .pending_commits
            .get(&group_key)
            .ok_or_else(|| ChatError::Db("pending MLS owner Commit is unavailable".into()))?;
        if pending.commit_hash != control.commit_hash
            || pending.epoch_before != block.epoch_before
            || pending.epoch_after != block.epoch_after
        {
            return Err(ChatError::Db(
                "pending MLS Commit differs from owner retry material".into(),
            ));
        }
        let mut group = MlsGroup::load(provider.storage(), &GroupId::from_slice(mls_group_id))
            .map_err(|error| mls_error("load MLS group", error))?
            .ok_or_else(|| ChatError::MissingKeyMaterial("MLS group is unavailable".into()))?;
        if group.epoch().as_u64() != pending.epoch_before || group.pending_commit().is_none() {
            return Err(ChatError::Trust(
                "durable MLS pending state does not match owner control".into(),
            ));
        }
        group
            .merge_pending_commit(&provider)
            .map_err(|error| mls_error("merge pending MLS owner commit", error))?;
        let private_control = extract_private_control_state(group.extensions())?;
        let conversation = metadata
            .conversations
            .get_mut(&block.conversation_id.to_string())
            .ok_or_else(|| ChatError::Db("local MLS conversation is unavailable".into()))?;
        if private_control.proposal_id != Some(block.proposal.proposal_id)
            || private_control.height != block.height
            || private_control.epoch != block.epoch_after
            || private_control.roster != control.next_roster
            || private_control.authority_set != conversation.current_authority_set
            || private_control.owner_set != control.owner_change.next_owner_set
            || conversation.last_finalized_height.checked_add(1) != Some(block.height)
            || conversation.last_finalized_epoch != block.epoch_before
            || conversation.last_block_hash != block.previous_block_hash
        {
            return Err(ChatError::Trust(
                "merged MLS private state differs from the owner block".into(),
            ));
        }
        conversation.last_finalized_height = block.height;
        conversation.last_finalized_epoch = block.epoch_after;
        conversation.last_block_hash = Some(block_hash);
        conversation.current_roster = control.next_roster;
        conversation.current_owner_set = control.owner_change.next_owner_set;
        let conversation = conversation.clone();
        let (local_address, _) = parse_device_credential_identity(&metadata.credential_identity)?;
        let retained_local_owner = group_owner_credential(&metadata, mls_group_id)
            .ok()
            .is_some_and(|owner| {
                conversation.current_roster.iter().any(|member| {
                    member.address.canonical() == local_address
                        && member.owner_id.as_deref() == Some(owner.owner_id.as_str())
                })
            });
        if !retained_local_owner {
            if let Some(seed) = metadata.group_owner_private_keys.remove(&group_key) {
                metadata
                    .group_owner_candidate_private_keys
                    .entry(group_key.clone())
                    .or_insert(seed);
            }
        }
        metadata.pending_commits.remove(&group_key);
        metadata.pending_owner_changes.remove(&group_key);
        let group = local_group_state(&group);
        let state = snapshot_provider(&provider, &metadata)?;
        self.db
            .apply(&Pending {
                mls_state: Some(state),
                ..Pending::default()
            })
            .await?;
        Ok(FinalizedMlsOwnerChange {
            group,
            conversation,
        })
    }
}

/// Remove prospective owner credentials for accounts that are no longer in
/// the authenticated private roster. Candidate material grants no role, but
/// retaining it after removal both violates the durable-state invariant and
/// could let a later re-addition reuse stale proof-of-possession material.
pub(super) fn prune_owner_candidates_for_roster(
    metadata: &mut SnapshotMetadata,
    mls_group_id: &[u8],
    roster: &[MlsConversationMemberV1],
) -> Result<()> {
    validate_group_id(mls_group_id)?;
    validate_group_roster(roster)?;
    let group_key = BASE64.encode(mls_group_id);
    let retained_accounts = roster
        .iter()
        .map(|member| member.address.canonical())
        .collect::<BTreeSet<_>>();
    let remove_candidate_map =
        if let Some(candidates) = metadata.owner_candidates.get_mut(&group_key) {
            candidates.retain(|account, _| retained_accounts.contains(account));
            candidates.is_empty()
        } else {
            false
        };
    if remove_candidate_map {
        metadata.owner_candidates.remove(&group_key);
    }
    let (local_address, _) = parse_device_credential_identity(&metadata.credential_identity)?;
    if !retained_accounts.contains(&local_address) {
        metadata
            .group_owner_candidate_private_keys
            .remove(&group_key);
    }
    Ok(())
}

fn candidate_conversation_id(content: &ChatContent) -> Result<Uuid> {
    let body: MlsGroupControlBodyV1 = serde_json::from_value(content.body.clone())
        .map_err(|error| ChatError::Content(error.to_string()))?;
    match body {
        MlsGroupControlBodyV1::OwnerCandidate { candidate } => Ok(candidate.conversation_id),
        MlsGroupControlBodyV1::OwnerApprovalRequest { request } => {
            Ok(request.proposal.conversation_id)
        }
        MlsGroupControlBodyV1::OwnerApproval { approval } => Ok(approval.conversation_id),
    }
}

fn validate_pending_owner_change(
    control: &PendingMlsOwnerChange,
    current_owners: &MlsOwnerSetV1,
) -> Result<()> {
    control.validate_durable()?;
    control
        .vote_request
        .block
        .owner_approval
        .as_ref()
        .ok_or_else(|| ChatError::Db("owner change has no approval certificate".into()))?
        .verify_partial(
            &control.vote_request.block.proposal,
            control.vote_request.block.transition_digest.as_deref(),
            current_owners,
        )
        .map_err(ChatError::Trust)
}

pub(super) fn validate_owner_role_transition(
    previous: &[MlsConversationMemberV1],
    next: &[MlsConversationMemberV1],
    current_owners: &MlsOwnerSetV1,
    next_owners: &MlsOwnerSetV1,
) -> Result<()> {
    let previous_by_address = previous
        .iter()
        .map(|member| (member.address.canonical(), member))
        .collect::<BTreeMap<_, _>>();
    let next_by_address = next
        .iter()
        .map(|member| (member.address.canonical(), member))
        .collect::<BTreeMap<_, _>>();
    if previous_by_address.len() != previous.len()
        || next_by_address.len() != next.len()
        || previous_by_address.keys().ne(next_by_address.keys())
    {
        return Err(ChatError::Invalid(
            "MLS owner change cannot add, remove, or replace members".into(),
        ));
    }
    let mut changed = false;
    for (address, current) in &previous_by_address {
        let proposed = next_by_address[address];
        if current.is_admin != proposed.is_admin {
            return Err(ChatError::Invalid(
                "MLS owner change cannot alter administrator roles".into(),
            ));
        }
        changed |= current.owner_id != proposed.owner_id;
    }
    if !changed || current_owners == next_owners {
        return Err(ChatError::Invalid(
            "MLS owner change must alter the exact owner mapping".into(),
        ));
    }
    validate_owner_set_matches_roster(next_owners, next).map_err(ChatError::Invalid)
}

pub(super) fn promote_local_owner_candidate(
    metadata: &mut SnapshotMetadata,
    mls_group_id: &[u8],
    private_control: &MlsPrivateControlStateV1,
) -> Result<()> {
    let group_key = BASE64.encode(mls_group_id);
    let (local_address, _) = parse_device_credential_identity(&metadata.credential_identity)?;
    let Some(owner_id) = private_control
        .roster
        .iter()
        .find(|member| member.address.canonical() == local_address)
        .and_then(|member| member.owner_id.as_deref())
    else {
        return Ok(());
    };
    if let Some(seed) = metadata.group_owner_private_keys.get(&group_key) {
        let credential = owner_credential_from_seed(seed)?;
        if credential.owner_id != owner_id {
            return Err(ChatError::Trust(
                "new MLS owner role differs from the active local owner key".into(),
            ));
        }
        return Ok(());
    }
    let seed = metadata
        .group_owner_candidate_private_keys
        .remove(&group_key)
        .ok_or_else(|| {
            ChatError::MissingKeyMaterial(
                "new MLS owner role has no locally generated candidate key".into(),
            )
        })?;
    let credential = owner_credential_from_seed(&seed)?;
    let declared = private_control
        .owner_set
        .owner(owner_id)
        .ok_or_else(|| ChatError::Trust("new local owner is absent from the owner set".into()))?;
    if credential.owner_id != owner_id
        || declared.public_key != BASE64.encode(&credential.public_key)
    {
        return Err(ChatError::Trust(
            "new MLS owner role differs from the local candidate key".into(),
        ));
    }
    metadata.group_owner_private_keys.insert(group_key, seed);
    Ok(())
}

fn validate_owner_set_matches_roster(
    owners: &MlsOwnerSetV1,
    roster: &[MlsConversationMemberV1],
) -> std::result::Result<(), String> {
    owners.validate()?;
    let roster_ids = roster
        .iter()
        .filter_map(|member| member.owner_id.as_deref())
        .collect::<BTreeSet<_>>();
    let declared_ids = owners
        .owners
        .iter()
        .map(|owner| owner.owner_id.as_str())
        .collect::<BTreeSet<_>>();
    if roster_ids != declared_ids {
        return Err("MLS owner set differs from the encrypted roster mapping".into());
    }
    Ok(())
}

pub(super) fn validate_new_owner_candidates(
    metadata: &SnapshotMetadata,
    conversation: &LocalMlsConversationRecord,
    next_roster: &[MlsConversationMemberV1],
    next_owners: &MlsOwnerSetV1,
    group_key: &str,
) -> Result<()> {
    let previous = conversation
        .current_roster
        .iter()
        .map(|member| (member.address.canonical(), member.owner_id.as_deref()))
        .collect::<BTreeMap<_, _>>();
    let candidates = metadata.owner_candidates.get(group_key);
    for member in next_roster {
        let address = member.address.canonical();
        let Some(owner_id) = member.owner_id.as_deref() else {
            continue;
        };
        if previous.get(&address).copied().flatten() == Some(owner_id) {
            continue;
        }
        let candidate = candidates
            .and_then(|entries| entries.get(&address))
            .ok_or_else(|| {
                ChatError::Trust(format!(
                    "MLS owner promotion for {address} has no authenticated candidate"
                ))
            })?;
        candidate.verify().map_err(ChatError::Trust)?;
        let declared = next_owners
            .owner(owner_id)
            .ok_or_else(|| ChatError::Trust("new MLS owner is absent from its owner set".into()))?;
        if candidate.conversation_id != conversation.request.genesis.conversation_id
            || candidate.incarnation != conversation.request.genesis.incarnation
            || candidate.account.canonical() != address
            || candidate.owner_id != owner_id
            || candidate.public_key != declared.public_key
        {
            return Err(ChatError::Trust(
                "MLS owner promotion differs from the authenticated member candidate".into(),
            ));
        }
    }
    Ok(())
}

fn owner_credential_from_seed(seed: &[u8]) -> Result<MlsGroupOwnerCredential> {
    let seed: [u8; 32] = seed
        .try_into()
        .map_err(|_| ChatError::Db("invalid MLS owner seed".into()))?;
    let signer = Ed25519SigningKey::from_bytes(&seed);
    let public_key = signer.verifying_key().to_bytes().to_vec();
    Ok(MlsGroupOwnerCredential {
        owner_id: hex::encode(Sha256::digest(&public_key)),
        public_key,
    })
}
